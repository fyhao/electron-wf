...
module.exports = {
	spec : function() {
		return {
			...
			{type:'string',name:'sortByDateTime',description:'Sort files by last modified time: asc or desc (default: asc)'}
			]
		}
	}
	,
...
var sortByDateTime = function(result, sortBy) {
	...
	result.sort(function(a,b) {
		var modifiedA = fs.statSync(a).mtime.getTime();
		var modifiedB = fs.statSync(b).mtime.getTime();
		return sortBy === 'desc' ? modifiedB - modifiedA : modifiedA - modifiedB;
	});
}
