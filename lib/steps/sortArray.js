...
var sortWithCustomCompare = function(ctx, step, array, next) {
	...
	var compareWorkFlow = ctx.config.workFlows[step.compareWf];
	if(typeof compareWorkFlow === 'undefined') {
		process.nextTick(function() {
			next(new Error('Compare workflow [' + step.compareWf + '] is not defined'));
		});
		return;
	}
	
	// Use a simple bubble sort algorithm to sort the array with custom compare
	// Note: Bubble sort has O(n²) complexity but is straightforward to implement
	// with asynchronous workflow execution. For typical workflow use cases with
	// modest array sizes, this is acceptable. For large arrays, users should use
	// the built-in asc/desc ordering instead of custom compare functions.
	var n = array.length;
	var i = 0;
		...
		if(i < n - 1) {
			...
			var innerLoop = function() {
				if(j < n - i - 1) {
					...
					var inputVars = {};
					for(var varName in ctx.vars) {
						if(Object.prototype.hasOwnProperty.call(ctx.vars, varName)) {
							inputVars[varName] = ctx.vars[varName];
						}
					}
					for(var s in step) {
						inputVars[s] = step[s];
					...
					ctx.executeWorkFlow(compareWorkFlow, {inputVars:inputVars,outputVars:step.outputVars,assert:ctx.opts.assert}, function(outputOpts) {
						if(outputOpts.error) {
							process.nextTick(function() {
								next(outputOpts.error);
							});
							return;
						}
						var compareResult = 0;
